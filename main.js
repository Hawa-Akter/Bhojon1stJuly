...
ipcMain.on("PosOrder:orderDetailsData", (e, result) => {
  ...
  AND VarientList.menuid = ? `;
    ...
    //gettingMenuVarient(menuId, varientId);
    //console.log("itemDetials", menuId, varientId);
  });
});
// function gettingMenuVarient(menuId, varientId) {
//   const db = new sqlite3.Database("./database.sqlite");
//   const menuVarientData = `SELECT FoodList.ProductName, VarientList.variantName
//   FROM VarientList,FoodList
//   WHERE  VarientList.menuid = FoodList.ProductsID
//   AND VarientList.variantid = ?
//   AND VarientList.menuid = ? `;
...
//   });
// }
ipcMain.on("orderDetailsBtn:id", (e, id) => {
  let orderDetailsWin;
  const db = new sqlite3.Database("./database.sqlite");
  ...
  orderDetailsWin.webContents.on("did-finish-load", () => {
    ...
    orderDetailsWin.webContents.send("Posorder:orderItemInfo", orderItemData);
  });
});
